...
int main() { 
    ...
    // by casting it as (byte_pointer), we're telling the compiler to treat this address as a pointer to a sequence of bytes, rather than as a pointer to a object or in this case a character.     
